...
class TraeBrowserRegister:
    ...
    def _register_oauth(self, email: str, password: str) -> dict:
        ...
        with OAuthBrowser(
            ...
            if not _wait_for_url(ob.active_page(), "trae.ai", timeout=self.manual_oauth_timeout):
                raise RuntimeError(f"Trae 浏览器登录未在 {self.manual_oauth_timeout} 秒内完成")
            page = ob.active_page()
            time.sleep(2)
            token, user_id, region = _get_trae_cloudide_token(page, self.log)
            resolved = finalize_oauth_email(email, email, "Trae")
            return {
                "email": resolved, "password": password,
                "token": token, "user_id": user_id,
                "region": region, "cashier_url": "", "ai_pay_host": "",
            }
